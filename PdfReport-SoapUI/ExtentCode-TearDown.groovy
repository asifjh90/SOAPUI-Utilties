...
HashMap<String, String> DBTableMapper = new HashMap<String, String>();
...
runner.getResults().each { testSuiteRunner ->
	...
	if(testSuiteRunner.getStatus().toString().equalsIgnoreCase("FAILED")) {
		failedTSCount++;
	}
	...
	parentTest.assignCategory(categoryName)
	...
	testSuiteRunner.getResults().each { testCaseRunner ->
		...
		if(testCaseRunner.getStatus().toString().equalsIgnoreCase("FAILED")){
			failedTCCount++;
		}
		...
		testCaseRunner.getResults().each { testStepResult ->
			...
			def ResFile = reportPath+"//"+tempTCName+"_"+tempStpName+"_Failed.txt"
...
DBTableMapper.put("DashboardTS", failedTSCount.toString());
DBTableMapper.put("DashboardTC", failedTCCount.toString());
PdfCreatorUtils.pdfDBTableWriter(DBTableMapper);
DBTableMapper.clear();

log.info "Total Failed TestSuites are : $failedTSCount"
log.info "Total Failed TestCases are : $failedTCCount";
